...
**Wichtige methodische Einschränkung**: Der Schärfe-Score wird pro Batch perzentil-normalisiert (relativ zum eigenen 5./95. Perzentil dieses Durchgangs) — zwei Shootings mit demselben Schärfe-Score-Durchschnitt sind dadurch nicht zwangsläufig absolut gleich scharf. Deshalb vergleicht das Resümee bewusst **keinen** Schärfe-Trend über mehrere Shootings hinweg (nur Belichtung, Emotion-Anteil und Hochformat-Anteil, die auf festen/absoluten Maßstäben beruhen). **Korrektur eines echten Bugs:** Eine reine Perzentil-Streckung hätte bei einem kleinen oder gleichmäßig scharfen Batch (z. B. wenige, alle tatsächlich scharfe Fotos aus einem Lightroom-Import) die winzigen, im Grunde bedeutungslosen Unterschiede zwischen den Fotos künstlich auf die volle 0–100-Skala gezogen — das schärfste Foto bekäme 100, das (absolut gesehen genauso scharfe) am wenigsten scharfe Foto des Batches würde fälschlich als "blurry" eingestuft. `normalizeSharpnessScores` (`src/lib/scoring.ts`) vertraut der Perzentil-Streckung deshalb nur noch, wenn die Spannweite des Batches relativ zu dessen eigenem Spitzenwert groß genug ist (das am wenigsten scharfe Foto hat weniger als etwa die Hälfte der Rohschärfe des schärfsten); darunter wird Richtung eines neutralen "scharf genug"-Werts gedämpft, statt eine Streckung aus reinem Rauschen zu erzeugen. Das ist eine begründete Heuristik, keine kalibrierte absolute Schärfe-Messung — Laplace-Varianz bleibt stark motivabhängig (ein detailreiches Motiv liest sich bei gleicher tatsächlicher Schärfe "schärfer" als ein ruhiges), weshalb es weiterhin keinen universellen absoluten Schwellenwert gibt.
...
Über "Poster erstellen" in der Ergebnisansicht lassen sich aus der aktuellen Auswahl automatisch fertig gestaltete Poster erzeugen — komplett clientseitig (Canvas/SVG-Rendering via `html-to-image`), kein neuer Backend-Baustein.

- **Kein erfundener Event-Typ**: Statt eine unzuverlässige Szenen-/Objekterkennung vorzutäuschen (Hochzeit vs. Business-Event vs. Konzert lässt sich aus Farbe/Schärfe/Gesichtern nicht seriös unterscheiden), nutzt das Poster den bereits gewählten Verwendungszweck/Profil als Titel — keine Doppelarbeit, keine falschen Behauptungen.
- **Farbstimmung & Personenformat** stammen aus bereits vorhandenen Signalen (semantische Tags wie `bw`/`warmColor`/`coolColor`/`vividColor`, Gesichtsanzahl/`crowdLikely`/`groupPhotoLikely`) — aggregiert über die ausgewählten Fotos, keine neue Erkennung.
- **8 strukturell unterschiedliche Templates** (nicht nur Farbvarianten): Minimalistisch, Bold Duoton, Editorial, Cinematic/Full-Bleed, Scrapbook, Filmstreifen, Raster, Collage. Die fotolastigeren Layouts erscheinen nur, wenn genug Fotos für ein sinnvolles Layout vorhanden sind (`minPhotos` je Template).
- **Kreatives Redesign (Nutzer-Feedback "zu wenig kreativ")**: Die Nutzerin hat 20 Referenzbilder (Canva-/Instagram-Postervorlagen) hochgeladen, die selbst klassifiziert und als grobe Stilrichtung übernommen wurden — kein 1:1-Nachbau, sondern Inspiration plus eigene Varianten:
  - *Bold Duoton* (ersetzt das vorherige, generische Typografie-Template): Duoton-Vollbild-Hero (Graustufen + Kontrast, mit der Foto-eigenen Akzentfarbe per `mix-blend-mode` eingefärbt statt einer fest codierten Marken-Farbe), fette Headline auf einem Eckenschleier — angelehnt an die Sport-Editorial-Collagen aus den Referenzen.
  - *Collage*: unregelmäßiges Foto-Raster (unterschiedlich große Kacheln statt gleichmäßigem Kontaktabzug) mit querliegendem Titelband — angelehnt an die Puzzle-artigen Grid-Poster aus den Referenzen.
  - *Scrapbook*: überlappende "abgerissene" Foto-Ausschnitte (Zickzack-Kante per CSS `clip-path`) auf strukturiertem Papierhintergrund, rotiertes Stempel-Badge, mehrschichtige Typografie — angelehnt an die Scrapbook-/Reiseposter-Referenzen.
- **Crop-Fix (Nutzer-Feedback "nicht essenzielle Dinge werden abgeschnitten")**: Alle Templates mit `object-fit: cover` positionieren das Bild jetzt anhand des tatsächlich erkannten Gesichts-Zentrums (`subjectCenter`, aus der bereits vorhandenen Gesichtserkennung in `faceAnalysis.ts` — die Bounding-Box wurde bisher nur für die Schärfe-Messung genutzt, nie für die Position). Kein neues Erkennungsmodell, nur eine bislang verworfene Information wird jetzt tatsächlich verwendet. Ohne erkanntes Gesicht: neutraler, leicht oberhalb der Mitte liegender Fallback-Punkt statt exaktem Center-Crop.
- **Farbpalette** wird aus dem Durchschnittsfarbwert der ausgewählten Fotos abgeleitet (`colorStats`, bereits Teil der Analyse), mit automatischer Hell/Dunkel-Textfarbwahl für Lesbarkeit (inkl. separater Kontrastwahl für Text auf der Akzentfarbe, da deren Helligkeit unabhängig vom Gesamt-Hintergrund ist).
- **Export**: Vorschau klein und günstig (schnelles Durchklicken der Varianten), erst beim Download wird die gewählte Variante einmalig in voller Druckauflösung (A3 bei ~300dpi, 3508×4961px) gerendert und als PNG heruntergeladen.
