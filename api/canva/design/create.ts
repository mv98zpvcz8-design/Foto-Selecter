...
/**
 * Receives the rendered poster's PNG bytes directly in the request body
 * (the Canva Connect API calls need our OAuth bearer token, which the
 * browser never holds, so this can't be called straight from the client
 * the way the old Adobe Express embed was) — uploads it as a Canva asset,
 * creates a design from it, and returns the edit_url for the client to open.
 */
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const assetId = await uploadAssetAndWait(active.session.accessToken, image, `poster-${Date.now()}.png`);
